...
$page_title = 'تعديل النشاط التجاري - Syriazzle';
...
try {
    // 1. جلب بيانات المتجر
    $stmt = $pdo->prepare("SELECT * FROM businesses WHERE id = ? AND user_id = ? AND deleted_at IS NULL");
    $stmt->execute([$business_id, $current_user_id]);
    $business = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$business) die("خطأ: ليس لديك صلاحية لتعديل هذا النشاط التجاري.");
...
<head>
    ...
    <title>تعديل: <?php echo htmlspecialchars($business['name']); ?></title>
    ...
    <link rel="stylesheet" href="css/add_business_form.css"> <!-- نستخدم نفس ملف التنسيق -->
    ...
    <style>
        /* === تنسيقات خاصة بنظام الطي والبحث (مدمجة لضمان العمل) === */
        
        /* 1. البحث وشريط الأدوات */
        .items-toolbar {
        ...
        .menu-item-entry.collapsed .delete-existing-item {
            display: none !important;
        }

        /* 3. الملخص (يظهر فقط عند الطي) */
        .collapsed-summary {
            display: none; width: 100%; align-items: center; gap: 15px;
        }
    ...
    <div class="form-container" style="padding-bottom: 0;">
        <?php if (isset($_SESSION['message'])): ?>
            <div style="padding: 15px; margin-bottom: 20px; border-radius: 8px; font-weight: bold;
                        background-color: <?php echo $_SESSION['message_type'] == 'success' ? '#d4edda' : '#f8d7da'; ?>;
                        color: <?php echo $_SESSION['message_type'] == 'success' ? '#155724' : '#721c24'; ?>;
                        border: 1px solid <?php echo $_SESSION['message_type'] == 'success' ? '#c3e6cb' : '#f5c6cb'; ?>;">
                <?php 
        ...
        <form id="edit-business-form" action="php/update_business_user.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="tab-pane active" id="step-0">
                <div class="form-section">
                    <h2><i class="fas fa-store"></i> المعلومات الأساسية</h2>
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select id="category" name="category" required>
                                <?php foreach (array_keys($dynamic_fields_config) as $category_name): ?>
                                    <option value="<?php echo htmlspecialchars($category_name); ?>" <?php if($business['category'] === $category_name) echo 'selected'; ?>><?php echo htmlspecialchars($category_name); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group"><label>المدينة <span style="color:red">*</span></label><input type="text" id="city" name="city" value="<?php echo htmlspecialchars($business['city']); ?>" required></div>
                                            <!-- === تحسين حقل العملة === -->
                    ...
                    <!-- ======================== -->
                        <div class="form-group full-width"><label>العنوان التفصيلي</label><input type="text" id="address" name="address" value="<?php echo htmlspecialchars($business['address'] ?? ''); ?>"></div>
                        ...
                        <div class="form-group"><label>رابط فيديو</label><input type="url" id="video_url" name="video_url" placeholder="https://..." value="<?php echo htmlspecialchars($business['video_url'] ?? ''); ?>"></div>
                    </div>
                    ...
                    <div class="form-group full-width"><label>وصف قصير</label><textarea id="description" name="description"><?php echo htmlspecialchars($business['description'] ?? ''); ?></textarea></div>
                </div>
            </div>
                ...
                <div id="dynamic-fields-container">
                    <?php foreach ($dynamic_fields_config as $category_name => $fields): ?>
                        <div id="wrapper-<?php echo str_replace(' ', '_', $category_name); ?>" class="dynamic-fields-wrapper form-section">
                            ...
                            <div class="form-grid">
                                <?php foreach ($fields as $field): $currentValue = $details_from_db[$field['name']] ?? ''; ?>
                                    <div class="form-group">
                                        <label><?php echo htmlspecialchars($field['name']); ?></label>
                                        <?php if ($field['type'] === 'select'): ?>
                                            <select name="details[<?php echo htmlspecialchars($field['name']); ?>]">
                                                <option value="">-- اختر --</option>
                                                <?php foreach ($field['options'] as $option): ?>
                                                    <option value="<?php echo htmlspecialchars($option); ?>" <?php if($currentValue === $option) echo 'selected'; ?>><?php echo htmlspecialchars($option); ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        <?php else: ?>
                                            <input type="<?php echo $field['type']; ?>" name="details[<?php echo htmlspecialchars($field['name']); ?>]" value="<?php echo htmlspecialchars($currentValue); ?>">
                                        <?php endif; ?>
                                    </div>
                                <?php endforeach; ?>
                            </div>
            ...
            <div class="tab-pane" id="step-2">
                        ...
                        <div class="form-group"><label>الشعار (Logo)</label>
                            <div class="image-uploader-group">
                                <div class="image-uploader-box">
                                    <input type="file" id="logo_image_input" name="logo_image" accept="image/*">
                                    <div class="upload-content"><i class="fas fa-portrait upload-icon"></i><div class="upload-text">تغيير الشعار</div></div>
                                </div>
                        ...
                        <div class="form-group"><label>صورة الغلاف</label>
                            <div class="image-uploader-group">
                                <div class="image-uploader-box">
                                    ...
                                    <div class="upload-content"><i class="fas fa-image upload-icon"></i><div class="upload-text">تغيير الغلاف</div></div>
                                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="image-uploader-group">
                        <div class="image-uploader-box">
                            <input type="file" id="gallery_images_new_input" name="gallery_images_new[]" multiple accept="image/*">
                            <div class="upload-content"><i class="fas fa-photo-video upload-icon"></i><div class="upload-text">إضافة صور جديدة</div></div>
                        </div>
            ...
            <div class="tab-pane" id="step-3">
                <div class="form-section">
                    ...
                    <p style="margin-top: -15px; color: #6c757d;">5 صور كحد أقصى تظهر في أعلى الصفحة.</p>
                    <div class="image-uploader-group">
                        <div class="image-uploader-box">
                            <input type="file" id="offer_images_new_input" name="offer_images_new[]" multiple accept="image/*">
                            <div class="upload-content"><i class="fas fa-photo-video upload-icon"></i><div class="upload-text">إضافة صور عروض جديدة</div></div>
                        </div>
                        <div class="image-preview-container" id="offers-previews-container"></div>
                        <div id="delete-offers-container"></div>
                    </div>
                </div>
            </div>

            <!-- الخطوة 5: العروض (Deal Manager) -->
            <div class="tab-pane" id="step-4">
                <div class="form-section">
                    <h2><i class="fas fa-tags"></i> العروض والصفقات</h2>
                    <div id="deal-items-container"></div>
                    <button type="button" id="add-deal-item-btn"><i class="fas fa-plus"></i> إضافة عرض جديد</button>
                </div>
            </div>

            <!-- الخطوة 6: المنيو (Menu Manager) -->
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid-hours">
                        ...
                        foreach($days as $day): 
                            ...
                            if (!$is_closed && !$is_24) {
                                $parts = explode('-', $stored_val);
                                if (count($parts) >= 2) { $start_time_val = trim($parts[0]); $end_time_val = trim($parts[1]); }
                            }
                        ?>
                            <div class="hours-row" id="row-<?php echo $day; ?>">
                                <div class="day-label"><?php echo $day; ?></div>
                                <div class="status-toggle">
                                    <label><input type="checkbox" class="day-status-cb" <?php echo !$is_closed ? 'checked' : ''; ?> onchange="toggleHours('<?php echo $day; ?>')"><span>مفتوح</span></label>
                                </div>
                                <div class="time-inputs-group <?php echo $is_closed ? 'disabled' : ''; ?>" id="group-<?php echo $day; ?>">
                                    <div style="display: flex; flex-direction: column;">
                                        ...
                                        <input type="time" class="time-input start-time" value="<?php echo htmlspecialchars($is_24 ? '00:00' : $start_time_val); ?>" <?php echo $is_24 ? 'disabled' : ''; ?> onchange="updateHiddenInput('<?php echo $day; ?>')">
                                    </div>
                                    <span style="font-weight: bold; color: #888;">-</span>
                                    <div style="display: flex; flex-direction: column;">
                                        <span style="font-size: 11px; color: #888;">إلى</span>
                                        <input type="time" class="time-input end-time" value="<?php echo htmlspecialchars($is_24 ? '23:59' : $end_time_val); ?>" <?php echo $is_24 ? 'disabled' : ''; ?> onchange="updateHiddenInput('<?php echo $day; ?>')">
                                    </div>
                                    <label style="font-size: 12px; margin-right: 10px; display: flex; align-items: center; gap: 4px;">
                                        <input type="checkbox" class="all-day-cb" <?php echo $is_24 ? 'checked' : ''; ?> onchange="toggle24Hours('<?php echo $day; ?>')"> 24 ساعة
                                    </label>
                                </div>
    ...
    <script>
        ...
        function nextPrev(n) {
            ...
            if (nextStep >= x.length) {
                ...
                document.getElementById("submitBtn").innerHTML = "جارٍ الحفظ...";
                return false;
            }
            currentTab = nextStep;
            showTab(currentTab);
        ...
        // 🔥 دالة التحقق الذكية (تفتح البطاقات المطوية إذا كان فيها خطأ)
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // 🔥 رفع الصور (مع إصلاح مشكلة المسح) 🔥
            function setupImageManager(config) {
                ...
                function createPreview(fileOrUrl, id = null, initialLoad = false) {
                    ...
                    if (config.maxFiles && (newFiles.files.length + existingFileCount >= config.maxFiles) && !isExisting && !initialLoad) {
                        alert(`لا يمكن رفع أكثر من ${config.maxFiles} صور.`); return null;
                    }
                    ...
                    wrapper.querySelector('.delete-btn').addEventListener('click', () => {
                        if (isExisting) {
                            if (deleteContainer) {
                                const hiddenInput = document.createElement('input'); hiddenInput.type = 'hidden'; hiddenInput.name = config.deleteHiddenInputName; hiddenInput.value = id; deleteContainer.appendChild(hiddenInput);
                            }
                            if (deleteSingleInput) { deleteSingleInput.value = '1'; }
                            existingFileCount--;
                        } else {
                            const updatedFiles = Array.from(newFiles.files).filter(f => f !== fileOrUrl);
                            const dt = new DataTransfer(); updatedFiles.forEach(f => dt.items.add(f)); newFiles = dt; 
                            input.files = newFiles.files; // Sync logic
                        }
                        wrapper.remove();
                    });
                    return wrapper;
                ...
                // الحدث المصحح: إزالة سطر مسح القيمة
                input.addEventListener('change', (e) => {
                    if (!config.isMultiple) { 
                        previewContainer.innerHTML = ''; 
                        if (deleteSingleInput) deleteSingleInput.value = '0'; 
                    ...
                    // 🛑 تم إزالة السطر: e.target.value = null; لكي لا تمسح الملفات
                });
            }
            ...
            const savedLat = parseFloat(latInput.value) || 33.5138;
            const savedLon = parseFloat(lonInput.value) || 36.2765;
            ...
            const menuManager = (() => {
                ...
                function createItemEntry(itemData = null) {
                    const isExisting = itemData !== null && itemData.id !== undefined;
                    const id = isExisting ? itemData.id : `new_${newItemCounter++}`;
                    const itemName = safehtmlspecialchars(itemData?.item_name || '');
                    const itemPrice = safehtmlspecialchars(itemData?.price || '');
                    const itemImg = (isExisting && itemData.image_path) ? safehtmlspecialchars(itemData.image_path) : 'image/default_logo.webp';

                    const entryDiv = document.createElement('div');
                    entryDiv.className = `menu-item-entry ${isExisting ? 'collapsed' : ''}`;
                    entryDiv.innerHTML = `
                        <div class="collapsed-summary"><img src="${itemImg}" class="summary-img" alt=""><div class="summary-info"><span class="summary-name">${itemName || 'عنصر جديد'}</span><span class="summary-price">${itemPrice ? itemPrice + ' ل.س' : ''}</span></div><i class="fas fa-chevron-down expand-icon"></i></div>
                        ...
                        <div class="menu-item-category-manager">
                            ...
                            <div class="category-input-group"><input type="text" class="category-input" value="${safehtmlspecialchars(itemData?.category_name || '')}"><button type="button" class="add-category-btn">إضافة</button></div>
                            <div class="category-tags"></div>
                            <input type="hidden" name="${isExisting ? `menu_items[${id}][category]` : `menu_items_new[${id}][category]`}" class="hidden-category-input" value="${safehtmlspecialchars(itemData?.category_name || '')}">
                        </div>
                        <div class="menu-item-fields">
                            <div class="image-upload-wrapper">
                                <img src="${(isExisting && itemData.image_path) ? safehtmlspecialchars(itemData.image_path) : ''}" class="image-preview ${ (isExisting && itemData.image_path) ? 'has-image' : ''}">
                                <input type="file" name="${isExisting ? `menu_items_images[${id}]` : `menu_items_new_images[${id}]`}" accept="image/*" class="menu-image-input">
                    ...
                    entryDiv.querySelector('.btn-danger').addEventListener('click', (e) => {
                        e.stopPropagation();
                        if(isExisting) { if(confirm('حذف نهائي؟')) { entryDiv.querySelector('input[name="delete_menu_items[]"]').checked = true; entryDiv.style.display = 'none'; } } 
                    ...
                    entryDiv.querySelector('.menu-image-input').addEventListener('change', (e) => {
                        if (e.target.files[0]) {
                            ...
                            reader.onload = ev => { 
                                ...
                                entryDiv.querySelector('.image-preview').src = res; entryDiv.querySelector('.image-preview').classList.add('has-image');
                                entryDiv.querySelector('.summary-img').src = res;
                            };
                    ...
                    entryDiv.querySelector('.add-category-btn').addEventListener('click', (e) => {
                        ...
                        if(val) { userCategories.add(val); entryDiv.querySelector('.hidden-category-input').value = val; document.querySelectorAll('#menu-items-container .menu-item-entry').forEach(updateCategoryTags); }
                    });
                }

                addMenuItemBtn.addEventListener('click', () => { 
                    if (menuItemsContainer.querySelectorAll('.menu-item-entry:not(.collapsed)').length >= 20) { alert("⚠️ يرجى حفظ التعديلات ثم إضافة المزيد."); return; }
                    createItemEntry(null); 
                });
            ...
            const dealManager = (() => {
                ...
                const toolbarHTML = `
                    <div class="items-toolbar">
                        <div class="search-box-wrapper"><input type="text" id="deal-search" placeholder="ابحث في العروض..."></div>
                        <button type="button" class="toggle-all-btn" id="toggle-deal-btn">فتح/إغلاق الكل</button>
                    </div>
                `;
                ...
                document.getElementById('deal-search').addEventListener('input', (e) => {
                    const term = e.target.value.toLowerCase();
                    document.querySelectorAll('#deal-items-container .menu-item-entry').forEach(entry => {
                        const nameVal = entry.querySelector('.name-live-update').value.toLowerCase();
                        entry.style.display = nameVal.includes(term) ? (entry.classList.contains('collapsed') ? 'flex' : 'block') : 'none';
                    });
                });
                let allExpanded = false;
                document.getElementById('toggle-deal-btn').addEventListener('click', () => {
                    allExpanded = !allExpanded;
                    document.querySelectorAll('#deal-items-container .menu-item-entry').forEach(entry => {
                        if (allExpanded) entry.classList.remove('collapsed'); else entry.classList.add('collapsed');
                    });
                ...
                function updateDealCategoryTags(entry) {
                    ...
                    allTags.forEach(category => {
                        ...
                        tag.innerHTML = `<span>${category}</span>`;
                        if (category === currentSelected) tag.classList.add('selected');
                        tag.onclick = (e) => {
                            e.stopPropagation(); entry.querySelector('.hidden-category-input').value = category; entry.querySelector('.category-input').value = category;
                            entry.querySelectorAll('.category-tag').forEach(t => t.classList.remove('selected')); tag.classList.add('selected');
                ...
                function createDealEntry(dealData = null) {
                    const isExisting = dealData !== null && dealData.id !== undefined;
                    const id = isExisting ? dealData.id : `new_${newDealItemCounter++}`;
                    const dealName = safehtmlspecialchars(dealData?.deal_name || '');
                    const newPrice = safehtmlspecialchars(dealData?.new_price || '');
                    const dealImg = (isExisting && dealData.image_path) ? safehtmlspecialchars(dealData.image_path) : 'image/default_logo.webp';

                    const entryDiv = document.createElement('div');
                    entryDiv.className = `menu-item-entry deal-entry ${isExisting ? 'collapsed' : ''}`;
                    
                    entryDiv.innerHTML = `
                        <div class="collapsed-summary"><img src="${dealImg}" class="summary-img" alt=""><div class="summary-info"><span class="summary-name">${dealName || 'عرض جديد'}</span><span class="summary-price">${newPrice ? newPrice : '0'}</span></div><i class="fas fa-chevron-down expand-icon"></i></div>
                        <div class="remove-btn-wrapper"><button type="button" class="btn-danger"><i class="fas fa-trash-alt"></i></button></div>
                        <div class="menu-item-category-manager">
                            <label>فئة العرض</label>
                            <div class="category-input-group"><input type="text" class="category-input" value="${safehtmlspecialchars(dealData?.category_name || '')}"><button type="button" class="add-category-btn">إضافة</button></div>
                            <div class="category-tags"></div>
                            <input type="hidden" name="${isExisting ? `deals[${id}][category_name]` : `deals_new[${id}][category_name]`}" class="hidden-category-input" value="${safehtmlspecialchars(dealData?.category_name || '')}">
                        </div>
                        <div class="menu-item-fields deal-fields">
                            <div class="image-upload-wrapper">
                                <img src="${(isExisting && dealData.image_path) ? safehtmlspecialchars(dealData.image_path) : ''}" class="image-preview ${ (isExisting && dealData.image_path) ? 'has-image' : ''}">
                                <input type="file" name="${isExisting ? `deals_images[${id}]` : `deals_new_images[${id}]`}" accept="image/*" class="menu-image-input">
                                <i class="fas fa-camera upload-icon"></i>
                            </div>
                            <div class="fields-grid">
                    ...
                    entryDiv.querySelector('.btn-danger').addEventListener('click', (e) => {
                        e.stopPropagation();
                        if(isExisting) { if(confirm('حذف نهائي؟')) { entryDiv.querySelector('input[name="delete_deals[]"]').checked = true; entryDiv.style.display = 'none'; } } 
                        else { entryDiv.remove(); }
                    });
                    entryDiv.querySelector('.menu-image-input').addEventListener('change', (e) => {
                        if (e.target.files[0]) {
                            const reader = new FileReader();
                            reader.onload = ev => { 
                                entryDiv.querySelector('.image-preview').src = ev.target.result; entryDiv.querySelector('.image-preview').classList.add('has-image');
                                entryDiv.querySelector('.summary-img').src = ev.target.result;
                            };
                            reader.readAsDataURL(e.target.files[0]);
                        }
                    });
                    entryDiv.querySelector('.add-category-btn').addEventListener('click', (e) => {
                        e.stopPropagation(); const val = entryDiv.querySelector('.category-input').value.trim();
                        if(val) { userDealCategories.add(val); entryDiv.querySelector('.hidden-category-input').value = val; document.querySelectorAll('#deal-items-container .menu-item-entry').forEach(updateDealCategoryTags); }
                    });
                }

                addDealItemBtn.addEventListener('click', () => {
                    if (dealItemsContainer.querySelectorAll('.menu-item-entry:not(.collapsed)').length >= 20) { alert("⚠️ يرجى حفظ التعديلات أولاً."); return; }
                    createDealEntry(null);
                });
            ...
            window.updateHiddenInput = function(day) {
                ...
                else hiddenInput.value = `${startTime ? startTime : '09:00'} - ${endTime ? endTime : '22:00'}`;
            };
            window.toggleHours = function(day) {
                const row = document.getElementById('row-' + day);
                const statusCb = row.querySelector('.day-status-cb');
                const inputsGroup = document.getElementById('group-' + day);
                if (statusCb.checked) { inputsGroup.classList.remove('disabled'); updateHiddenInput(day); } 
            ...
            window.toggle24Hours = function(day) {
                ...
                if (allDayCb.checked) { startInput.disabled = true; endInput.disabled = true; startInput.value = '00:00'; endInput.value = '23:59'; } 
                else { startInput.disabled = false; endInput.disabled = false; if(startInput.value === '00:00') startInput.value = '09:00'; if(endInput.value === '23:59') endInput.value = '22:00'; }
